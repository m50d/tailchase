...
trait UnfixHelper1 extends UnfixHelper2 {
  ...
  implicit def deriveInstance[F, G, H](implicit gen: Generic.Aux[G, H], cg: Lazy[UnfixHelper[F, Any, H]]) =
    new UnfixHelper[F, Any, G] with NonTrivial {
...
object UnfixHelper extends UnfixHelper1 {
  type Unfix[F, U[_]] = UnfixHelper[F, Any, F] {
    type O[A] = U[A]
  }
